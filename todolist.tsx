import { useState, useEffect, Fragment, SetStateAction, JSXElementConstructor, Key, ReactElement, ReactNode, ReactPortal } from 'react';
...
const Todolist = () => {
    ...
    const viewTask = (item: any = null) => {
        ...
        item.started = true; // Add this line
        setTimeout(() => {
            setViewTaskModal(true);
        });
    };

    const startTask = (task: SetStateAction<null>) => {
        stopActiveTask(); // Stop the previous task if any
        const updatedTasks = allTasks.map((t) => {
            if (t.id === task.id) {
                setActiveTask(task); // Set the new active task
                setTotalTime(0); // Reset total time for the new task
                return { ...t, started: true }; // Mark task as started
            }
            return t;
        });
        ...
        const intervalId = setInterval(() => {
            setTotalTime((prevTime) => prevTime + 1000); // Increment total time by 1 second
        }, 1000);
        setTimerInterval(intervalId);
        showMessage('Task has been started successfully.');
    };

    const completeTask = (task: { id: number }) => {
        const updatedTasks = allTasks.map((t) => {
            if (t.id === task.id) {
                clearInterval(timerInterval); // Stop the timer
                setTimerInterval(null); // Reset the timer interval
                return { ...t, completed: true }; // Mark task as completed
            }
            return t;
        });
        setAllTasks(updatedTasks);
    ...
    const assignTaskToUser = (task: { title: any }, p0: string) => {
        // Logic to assign the task to the user
        // ...

        // Notify the user about the new task assignment
        notifyUser(`A new task has been assigned to you: ${task.title}`);
    };

    const notifyUser = (message: any) => {
    ...
    const NotificationList = ({ notifications }) => {
        return (
            <div className="notification-list">
                {notifications.map(
                    (
                        notification: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Iterable<ReactNode> | null | undefined,
    ...
    return (
        <div>
            {/* Render tasks and other components */}
            <NotificationList notifications={notifications} />
            {/* Example of assigning a task */}
            <button onClick={() => assignTaskToUser(allTasks[0], 'User  A')}>Assign Task 1 to User A</button>
        </div>
    );
    ...
    <div className="flex justify-end items-center mt-8">
        ...
        <button type="button" className="btn btn-outline-success" onClick={() => startTask(selectedTask)}>
            Start Task
        </button>
        <button type="button" className="btn btn-outline-danger" onClick={() => completeTask(selectedTask)}>
            Complete Task
        </button>
                                                ...
                                                <div
                                                    className={`badge rounded-3xl capitalize ${
                                                        selectedTask.priority === 'medium'
                                                            ...
                                                            : selectedTask.priority === 'high'
                                                            ? 'badge-outline-danger '
                                                            : selectedTask.priority === 'very important'
                                                    }`}
                                                >
                                                    {selectedTask.priority}
                                                </div>
